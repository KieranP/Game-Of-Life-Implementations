...
- No support for continuous loops; fallback to recursive function calls (see
  `Play.loop`).
- No infinity literal or float max constant in the standard library; fallback to
  `1 / 0` (see `infinity` in Play.hs).
- No support for random numbers in the standard library; fallback to the
  `random` package (see `World.populateCells`).
