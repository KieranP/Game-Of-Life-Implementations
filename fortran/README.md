...
- Identifiers are case-insensitive, so where variables are `world` and `cell` in
  other implementations, they are `w` and `c` so as not to conflict with the
  `World` and `Cell` types.
- Identifiers cannot start with an underscore, so the `_f` helper is named `f`
  (see `f` in play.f90).
- No built-in HashMap, so a very basic implementation is included (see
  hashmap.f90).
