...
(defparameter CLEAR_SCREEN (format nil "~c[?2026h~c[H~c[2J" #\Escape #\Escape #\Escape))
(defparameter SHOW_SCREEN (format nil "~c[?2026l" #\Escape))
        ...
        (let ((avg-tick (/ total-tick (tick world))))
          ...
          (let* ((render-start (get-internal-real-time))
            ...
            (let ((avg-render (/ total-render (tick world))))
              ...
              (unless minimal
                (format t "~a~a~%" rendered SHOW_SCREEN)))))))))
