...
- No infinity literal or float max constant in the standard library; `/.` by
  zero yields 0.0, so fallback to a large-float sentinel (see `infinity` in
  play.gleam).
- No support for environment variables or monotonic time in the standard
  library; fallback to Erlang FFI (see play_ffi.erl).
